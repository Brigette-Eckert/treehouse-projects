...
def get_locations(cells):
    """ Randomly pick starting locations for the monster, the door and the player.  All locations must be unique.
...
#failing on purpose for example of failing docstring test
def get_moves(player):
    """ Based on the tuple of the player's current postion, return the list of acceptable moves
...
def move_player(player, move):
    """" Move player to new cell based on player input
...
def draw_map(cells):
    """ Draws out the game board based on the Game Dimensions
    >>> cells = build_cells(2,2)
    >>> GAME_DIMENSIONS = (2,2)
    >>> draw_map(cells)
     _ _ _ _ _
